...
function setup() {
  ...
  // dat.GUI 설정
  gui = new dat.GUI();
  gui.add(guiSettings, "sizeSmall", 1, 20, 1).name("Small Particle Size");
  gui.add(guiSettings, "sizeLarge", 2, 50, 1).name("Large Particle Size");
  gui.addColor(settings, "colorA").name("Bright Color");
  gui.addColor(settings, "colorB").name("Warm Color");
  gui.add(settings, "nearDist", 50, 200, 1).name("Near Reaction Distance");
  gui.add(settings, "midDist", 150, 400, 1).name("Mid Reaction Distance");
  gui.add(settings, "forceNear", 1, 10, 0.1).name("Near Force Strength");
  gui.add(settings, "forceMid", 0.01, 0.2, 0.01).name("Mid Restoring Force");
  ...
  for (let i = 0; i < lines.length; i++) {
    stencil.text(lines[i], width / 2, yOffset + i * 80);
  }
  stencil.loadPixels();
